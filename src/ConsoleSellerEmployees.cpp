...
std::string ConsoleSellerEmployees::execute_command() {
    ...
    switch (current_command){
        case EXIT:
            return "";
        case CHECK_DISMISS:
            argument = input<int>([](int x){return x>=-1;}, [](std::string str){return stoi(str);}, "Введите количество сотрудников, возможность увольнения которых Вы бы хотели узнать (-1 для возврата назад): ", "Допущена ошибка, повторите ввод: ");
            if (argument != -1) return seller->check_dismiss_employees(argument);
            else return "";
        case DISMISS:
            argument = input<int>([](int x){return x>=-1;}, [](std::string str){return stoi(str);}, "Введите количество сотрудников, которые Вы бы хотели уволить (-1 для возврата назад): ", "Допущена ошибка, повторите ввод: ");
            if (argument != -1) return seller->dismiss_employees(argument);
            else return "";
        case CHECK_HIRE:
            argument = input<int>([](int x){return x>=-1;}, [](std::string str){return stoi(str);}, "Введите количество сотрудников, возможность наёма которых Вы бы хотели узнать (-1 для возврата назад): ", "Допущена ошибка, повторите ввод: ");
            if (argument != -1) return seller->check_hire_employees(argument);
            else return "";
        case HIRE:
            argument = input<int>([](int x){return x>=-1;}, [](std::string str){return stoi(str);}, "Введите количество сотрудников, которые Вы бы хотели нанять (-1 для возврата назад): ", "Допущена ошибка, повторите ввод: ");
